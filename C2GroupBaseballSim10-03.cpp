...
// Prototype functions
void determineGameWinner (int[], int gameSize);
...
void determineGameWinner(int gameArray[], int gameSize){
	...
	for (int x = 0; x < gameSize; x++){
		gameArray[x] = inning(atBat(3, 0.2), atBat(4, 0.3), atBat(2, 0.23));
		if (gameArray[x] == 1){
			scoreCount++;
		}
	}
